...
import { IconLogin, IconSearch, IconLogout, IconCart, IconUser, IconMenu, IconClose } from '@/components/ui/Icons'
...
export default function PublicLayout() {
 ...
 <div className="bg-primary text-white text-sm text-center py-2 px-4 overflow-hidden tracking-wide">
 ...
 aria-label="Menú"
 >
 {showMobileMenu ? <IconClose size={24} /> : <IconMenu size={24} />}
 </button>

 {/* Search (desktop) */}
 <div className="hidden md:flex items-center gap-2 w-full max-w-[240px] relative" ref={searchRef}>
 <form onSubmit={handleSearch} className="relative w-full">
 <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"><IconSearch size={18} /></span>
 <input
 type="text"
 value={searchQuery}
 onChange={(e) => {
 ...
 className="w-full pl-9 pr-4 py-2 bg-gray-50 border border-gray-200 text-sm text-carbon placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary focus:bg-white transition-colors"
 />
 </form>
 ...
 {showUserMenu && (
 <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-gray-200 shadow-lg py-1 z-50">
 <Link
 to="/perfil"
 ...
 <Link to="/login" className="text-gray-500 hover:text-primary transition-colors" aria-label="Iniciar sesión">
 <IconLogin size={30} />
 </Link>
 )}
 <button
 ...
 <IconCart size={29} />
 {cartCount > 0 && (
 <span className="absolute -top-2 -right-2 bg-gold text-carbon text-[10px] font-bold w-5 h-5 flex items-center justify-center">
 ...
 <Link
 to={link.href}
 className="relative px-4 py-2.5 text-sm font-medium text-carbon hover:text-primary transition-all duration-200 hover:scale-105 uppercase after:absolute after:bottom-0 after:left-1/2 after:-translate-x-1/2 after:h-[2px] after:w-0 after:bg-primary after: after:transition-all after:duration-300 hover:after:w-full"
 >
 {link.label}
 </Link>
 </li>
 ...
 placeholder="Buscá tu reloj ideal..."
 className="w-full pl-9 pr-4 py-2.5 bg-gray-50 border border-gray-200 text-sm text-carbon placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary focus:bg-white transition-colors"
 />
 </form>
 </div>
 ...
 to={link.href}
 onClick={() => setShowMobileMenu(false)}
 className="block px-3 py-2.5 text-sm text-carbon hover:text-primary hover:bg-gray-50 transition-colors font-medium"
 >
 {link.label}
 </Link>
 </li>
 ...
 to="/admin"
 onClick={() => setShowMobileMenu(false)}
 className="flex items-center gap-2 px-3 py-2.5 text-sm text-primary hover:bg-gray-50 transition-colors font-medium"
 >
 📊 Panel Admin
 </Link>
 </>
 ...
 to="/perfil"
 onClick={() => setShowMobileMenu(false)}
 className="block px-3 py-2 text-sm text-carbon hover:text-primary hover:bg-gray-50 transition-colors"
 >
 Mi Perfil
 </Link>
 <Link
 ...
 >
 <IconLogout size={30} /> Cerrar Sesión
 </button>
 </div>
 ) : (
 <div className="space-y-2">
 ...
 to="/login"
 onClick={() => setShowMobileMenu(false)}
 className="block w-full text-center bg-primary text-white px-4 py-2.5 text-sm font-semibold hover:bg-primary-dark transition-colors"
 >
 Iniciar Sesión
 </Link>
 <Link
 to="/registro"
 onClick={() => setShowMobileMenu(false)}
 className="block w-full text-center border border-primary text-primary px-4 py-2.5 text-sm font-semibold hover:bg-gray-50 transition-colors"
 >
 Registrarse
 </Link>
 </div>
