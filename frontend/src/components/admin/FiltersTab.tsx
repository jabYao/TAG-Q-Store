...
export default function FiltersTab() {
 ...
 {/* New group form */}
 {creatingGroup && (
 <div className="bg-white p-5 border-2 border-primary shadow-sm mb-4 flex flex-col gap-3">
 <input
 type="text"
 ...
 className="bg-primary text-white px-4 py-2 text-sm font-semibold hover:bg-primary-dark"
 >
 Guardar grupo
 </button>
 <button
 ...
 {groups?.map((group) => (
 <div key={group.id} className="bg-white border border-gray-100 shadow-sm overflow-hidden">
 {/* Group header */}
 ...
 value={groupForm.name}
 onChange={(e) => setGroupForm((f) => ({ ...f, name: e.target.value }))}
 className="px-3 py-1.5 border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary flex-1 max-w-xs"
 autoFocus
 />
 <select
 value={groupForm.display_type}
 onChange={(e) => setGroupForm((f) => ({ ...f, display_type: e.target.value as 'checkbox' | 'radio' }))}
 className="px-3 py-1.5 border border-gray-200 text-sm"
 >
 ...
 className="bg-primary text-white px-3 py-1.5 text-xs font-semibold hover:bg-primary-dark"
 >
 Guardar
 </button>
 ...
 <div key={val.id} className="flex items-center gap-1 bg-gray-50 px-3 py-1.5 text-sm">
 ...
 className="w-28 px-2 py-0.5 border border-gray-200 text-xs focus:outline-none focus:ring-1 focus:ring-primary"
 autoFocus
 onKeyDown={(e) => {
 ...
 className="w-32 px-2 py-1.5 border border-gray-200 text-xs focus:outline-none focus:ring-1 focus:ring-primary"
 autoFocus
 onKeyDown={(e) => {
 ...
 className="text-xs text-primary hover:underline px-2 py-1.5 hover:bg-primary/5 transition-colors"
 >
 + Agregar valor
 </button>
 )}
 ...
 <div className="w-6 h-6 border-2 border-primary/30 border-t-primary animate-spin mx-auto" />
 </div>
 )}
 </div>
 </div>
