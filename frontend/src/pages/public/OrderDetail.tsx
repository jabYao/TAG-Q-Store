...
export default function OrderDetail() {
 ...
 <span className={`text-xs px-3 py-1 font-medium ${status.color}`}>
 {status.label}
 </span>
 </div>
 ...
 <div className={`w-3 h-3 ${i === statusesLen - 1 ? 'bg-primary' : 'bg-gray-300'}`} />
 {i < statusesLen - 1 && <div className="w-px h-full bg-gray-200 my-1" />}
 </div>
 <div className="pb-3">
 <span className={`text-[10px] px-2 py-0.5 font-medium ${st.color}`}>{st.label}</span>
