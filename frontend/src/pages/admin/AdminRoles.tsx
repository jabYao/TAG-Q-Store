...
export default function AdminRoles() {
 ...
 {roles.map((role: any) => (
 <div key={role.id} className="bg-white border border-gray-100 shadow-sm p-5">
 ...
 <span key={p} className="text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 ">{p}</span>
 ))}
 </div>
 ...
 <div className="flex flex-wrap gap-1">
 {perms.map((p: string) => (
 <label key={p} className={`text-[10px] px-2 py-0.5 cursor-pointer transition-colors ${
 ...
 className="mt-3 bg-primary text-white px-4 py-1.5 text-xs hover:bg-primary-dark transition-colors">
 Guardar permisos
 </button>
 </div>
 )}
 ...
 {users.map((user: any) => (
 <div key={user.id} className="bg-white border border-gray-100 shadow-sm p-4">
 ...
 <span key={r} className="text-[10px] bg-primary/10 text-primary px-1.5 py-0.5 capitalize">{r}</span>
 ))}
 </div>
 </div>
 ))}
