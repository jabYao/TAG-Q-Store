...
export default function AdminOrderDetail() {
 ...
 {order.order_number}
 {order.is_internal && <span className="ml-2 text-xs px-2 py-0.5 font-medium bg-purple-100 text-purple-700">Interna</span>}
 </h1>
 ...
 {/* Totals */}
 <div className="bg-white border border-gray-100 shadow-sm p-5 mt-6">
 ...
 {order.notes && <div className="mt-3 p-3 bg-gray-50 text-xs text-gray-500">📝 {order.notes}</div>}
 </div>
 ...
 className="px-4 py-2 text-xs font-semibold border transition-colors hover:bg-primary hover:text-white hover:border-primary disabled:opacity-50">
 {statusLabels[st] ?? st}
 </button>
 ))}
 </div>
 ...
 <div className={`w-2.5 h-2.5 ${i === order.statuses.length - 1 ? 'bg-primary' : 'bg-gray-300'}`} />
