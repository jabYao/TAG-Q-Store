...
export default function AdminHeroes() {
  ...
  return (
    <div className="p-6">
      ...
      {editingId !== null && (
        <div className="bg-white p-5 rounded-xl border-2 border-primary shadow-sm mb-6 max-w-2xl space-y-4">
          <h3 className="text-sm font-semibold text-carbon">{editingId === 0 ? 'Nuevo Hero' : 'Editar Hero'}</h3>
          <input type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
            placeholder="Título" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
          <input type="text" value={form.subtitle} onChange={e => setForm(f => ({ ...f, subtitle: e.target.value }))}
            placeholder="Subtítulo" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
          <div className="grid grid-cols-2 gap-4">
            <input type="text" value={form.cta_text} onChange={e => setForm(f => ({ ...f, cta_text: e.target.value }))}
              placeholder="Texto del botón" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
            <input type="text" value={form.cta_link} onChange={e => setForm(f => ({ ...f, cta_link: e.target.value }))}
              placeholder="Link del botón" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
          </div>
      ...
      <div className="space-y-4">
        {heroes?.map((hero) => (
          <div key={hero.id} className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm flex gap-6">
            ...
            <div className="flex-1">
              ...
              <button onClick={() => toggleMutation.mutate({ id: hero.id, is_active: !hero.is_active })}
                className={`mt-2 text-[10px] px-2 py-0.5 rounded-full font-medium ${
